...
export async function getTodayWeather(city: GeolocationResponse | undefined) {
	if (city) {
...
export async function getWeekWeather(city: GeolocationResponse | undefined) {
	if (city) {
		...
		const response: { daily: WeekWeatherType } = await (
			await fetch(url)
		).json();
